...
Canonical ergodic search~\cite{mathew2011metrics} does not deal with continuous exploration. It derives an agent's control so that its trajectory maximizes an ergodic metric defined in the spectral domain~\cite{calinon2020mixture}.
...
We will provide a solution to Problem~\ref{pb:enerergo} (see Sec.~\ref{sec:meth}), assuming that there are one or more areas in $\mathcal{Q}$ -- i.e., charging stations -- where the agents $\boldsymbol{\alpha}$ can land and recharge the battery, e.g., using wireless charging (see Sec.~\ref{sec:res}). 
...
\noindent
For the purposes of defining the spatial distribution in both Problem~\ref{pb:ergo} and \ref{pb:enerergo}, let us consider a Gaussian mixture model (GMM)
...
To formulate the solution to Problem~\ref{pb:enerergo}, let us first extend the OCP in Eq.~(\ref{eq:ocpergo}) to multiagent systems. Eq.~(\ref{eq:ocpergomin}) becomes
...
For the purpose of battery modeling, let us consider an equivalent circuit model (ECM) -- commonly employed in battery metrics estimation for robots and other applications, especially if equipped with rechargeable battery cells~\cite{zhang2018online,xiaosong2012comparative,hasan2018exogenous,hinz2019comparison,mousavi2014various,seewald2022energy}.

The ECM model we employ is a second-order RC model~\cite{zhao2017observability}, as illustrated in Figure~\ref{}. 
...
\end{equation}
where $R\in\mathbb{R}$ is the single resistor measured in ohm in Figure~\ref{}, and $V_{\text{OC}}$ is the open circuit voltage. It is a nonlinear function of the battery SoC and can be retrieved from the battery data sheet~\cite{hinz2019comparison}.
