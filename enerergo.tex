...
\noindent
In the second extensive scenario, four MAVs $\alpha_1$, $\alpha_2$, $\alpha_3$, and $\alpha_4$ are placed on top of four wireless charging stations. The optional constraints in Eq.~(\ref{eq:ocpfinalextra1}) are built so that each MAV covers two Gaussians at a time that are respectively farthest (see different colors in Fig.~\ref{fig:scenario}; green empty squares refer to the Gaussians covered by the ``red'' agent, cyan empty squares refer to the Guassians covered by the ``blue'' agent), whereas the battery constraint is edited so that there are two MAVs at each time, i.e.,
\begin{equation}
...
The battery and recharging coefficients are those used in the previous scenario, whereas the horizon is now set to two minutes and a half.
