...
%
In the remainder of the text, we will use the terms \textit{continuously} and \textit{uninterruptedly} to indicate that there is at least one agent exploring the space at all times, i.e., a certain reasonable level of coverage is already sattisfied.
...
\begin{defn}[Ergodicity]
  ...
  A trajectory $\mathbf{q}(t)\in\mathcal{Q}$ is said to be \textit{ergodic} with respect to a spatial distribution $\phi$ if and only if
  \begin{equation}
  ...
  \end{equation}
  for all Lebesgue functions $\Omega$~\cite{mathew2011metrics}. The function $\kappa$ maps the state to the exploration work space (see Sec.~\ref{sec:meth}).
  \end{defn}

\begin{defn}[Ergodic metric]\label{def:ergom}
  Consider a time average distribution that describes where the robot spends more time over a finite time window $[t_0-t_f]$, i.e., $h(\mathbf{q}(t))=\int_{\mathcal{T}}{\Delta\big(\mathbf{q}(t)\big)}\,dt\,/t_f$, $\Delta$ is a Dirac delta function, and $t_0,t_f\in\mathbb{R}_{>0}$ are the initial and final time instants. 
...
\begin{pb}[Energy-aware ergodic search]\label{pb:enerergo}
  Consider a set of $n$ agents $\boldsymbol{\alpha}:=\{\alpha_1,\alpha_2,\dots,\alpha_n\}$, a bounded space $\mathcal{Q}$, and a spatial distribution $\phi$ similar to Problem~\ref{pb:ergo}.~\textit{Energy-aware ergodic search problem} is the problem of deriving each agent ${}^j\alpha$ control action ${}^j\mathbf{u}(t)$ so that $\forall j,\,\,{}^j\mathbf{q}(t)$ induces a time average distribution that is proportional to the spatial distribution $\phi$ and %there is at least one agent exploring at all times.
  %$\exists \alpha_k\in\boldsymbol{\alpha},\,\,{}^k\mathbf{q}(t)$ s.t.  
  the ergodic metric calculated between the agents' averaged time average distribution in Definition~\ref{def:ergom} is satisfied, i.e., satisfying an upperbound on the ergodic metric 
...
\begin{figure}[b!]
  ...
  \caption{\textbf{Experimental evaluation of %intermittent exploration
  competing exploration with two Gaussians}. The problem in Fig.~\ref{fig:res2} is applied to two areas, but now with two areas. The agents compete for both areas with high information density, with the agents ``blue'' and ``red'' selected first. One can note how both agents swap between the areas but spend most time in proximity to the Gaussians. At the end of the first horizon, they return to the charging stations with the other two agents ``dark-green'' and ``magenta'' exploring the space.}
  \label{fig:res3}
\end{figure}
...
In the following horizon, the agents $\alpha_3$ and $\alpha_4$ are recharging whereas $\alpha_1$ and $\alpha_2$ proceed with the exploration. %from where they left off.%, and so on.
